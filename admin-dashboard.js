...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let securityEvents = {
    loginAttempts: Array(6)
      .fill(0)
      .map(() => Math.random() * 15 + 5),
    failedLogins: Array(6)
      .fill(0)
      .map(() => Math.random() * 5),
  };

  // Initialize charts
  const systemHealthChart = new Chart(
  ...
  const securityEventsChart = new Chart(
    document.getElementById("securityEventsChart"),
    {
      type: "line",
      data: {
  ...
  setInterval(() => {
    ...
    systemHealth.healthy = Math.max(
      0,
      Math.min(100, systemHealth.healthy + generateTrendingData(0, 0.2, 1))
    );
    systemHealth.warning = Math.max(
      0,
      Math.min(
        100 - systemHealth.healthy,
    ...
    Object.keys(dataUsage).forEach((category) => {
      dataUsage[category] = Math.max(
        0,
        Math.min(100, dataUsage[category] + generateTrendingData(0, 0.1, 0.5))
      );
    });

    // Update security events with realistic patterns
    securityEvents.loginAttempts = securityEvents.loginAttempts.map(
      (value, i) => detectAnomaly(generateTrendingData(value, 0.05, 0.3), 0.4)
    );
    ...
    if (modalSecurityEventsChart) {
      modalSecurityEventsChart.data.datasets[0].data =
        securityEvents.loginAttempts;
      modalSecurityEventsChart.data.datasets[1].data =
  ...
  function checkForAnomalies() {
    ...
    const lastLoginAttempts =
      securityEvents.loginAttempts[securityEvents.loginAttempts.length - 1];
    const lastFailedLogins =
      securityEvents.failedLogins[securityEvents.failedLogins.length - 1];
    ...
    if (lastLoginAttempts > 15) {
      addAlert(
        `Unusual login activity detected: ${lastLoginAttempts} attempts`
      );
    }
